...
const DAGGERHEART_ARMOR_NAME_ALIASES: Record<string, string> = {
  基础轻型制式装备: "填充布甲",
  基础制式装备: "皮甲",
  基础重型制式装备: "链甲",
  基础加重型制式装备: "全板甲",
...
export function resolvePresetArmor(
  id?: string,
  ruleSetId: SheetData["ruleSetId"] = "daggerheart",
): ArmorItem | undefined {
  if (!id) {
    return undefined
  }
...
export function inferArmorSelection(
  ...
  const preset = resolvePresetArmor(data.armorName, data.ruleSetId)
  if (
    preset &&
...
export function getPresetEquipmentEffectSources(
  ...
  if (armorSelection.mode === "preset") {
    const armor = resolvePresetArmor(
      armorSelection.id,
      (data as Partial<Pick<SheetData, "ruleSetId">>).ruleSetId,
    )
    if (armor?.effects) {
      sources.push({
        slot: "armor",
